 ...
 */
@WebServlet("/modifyBeosztas")
public class modifyBeosztas extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public modifyBeosztas() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String[][] nevekAssz = nameGetAssz();
		sqlParancsok haviidoszam = new sqlParancsok(request.getParameter("id"));
		beosztObject Object = new beosztObject(request.getParameter("id"));
		Map<String, String> myMap = new HashMap<String, String>();
		myMap = Object.returnMyMap();
		beosztasMethods hiddentable = new beosztasMethods();
		beosztObject assTimes = new beosztObject(request.getParameter("id"));

		String printout = teljesTabla(nevek, nevekAssz, myMap) + "<h1></h1>";
		printout = printout + tablaAlul(assTimes.monthMuszak, assTimes.monthTime) + "<h1></h1>";
		printout = printout + hiddentable.hiddenTable();
		response.getWriter().append(printout);

	}

	protected String elsoSor(Map<String, String> myMap) throws ServletException, IOException {
		String Year = myMap.get("Year");
		String Week = myMap.get("Week");
		...
		String eredmeny = "<h1></h1><form action=\"saveModifiedBeosztas.jsp\"><table id=\"beosztasTablazat\" style=\"width: 99%\" align=\"center\"> <tr>\r\n"
		...
		eredmeny = eredmeny + "		<th colspan=\"2\" id=\"date5\" class=\"jobbvastag topvastag\">"
				+ sdf.format(cal.getTime()) + "</th>\r\n" + "\r\n"
				+ "	</tr> <input type=\"hidden\" name=\"id\" value=\"" + myMap.get("id")
				+ "\"></input><input type=\"hidden\" name=\"date10\" id=\"date10\" value=\"" + sdf.format(cal.getTime())
	...
	protected String teljesSor(int foMellek, int rendelo, String[][] nevek, String[][] nevekAssz,
			Map<String, String> myMap) throws ServletException, IOException {
			...
			for (; j < 3; j++) {
				...
				if (foMellek == 1) {
					eredmeny = eredmeny + "<td class=\"topvastag2\" ID=\"" + id + "1\"" + ">"
							+ selectMenu("", Integer.parseInt(id) * 10 + 1, nevek, myMap) + "</td>\r\n";

					eredmeny = eredmeny + "<td class=\"jobbvastag2 topvastag2\" ID=\"" + id + "2" + "\">"
							+ selectMenuAssz("", Integer.parseInt(id) * 10 + 2, nevekAssz, myMap) + "</td>\r\n";
					if (i == 5) {
						break;
					}
				} else {
		...
		for (int i = 1; i < 6; i++) {
			...
			for (; j < 3; j++) {
				...
				eredmeny = eredmeny + "<td class=\"timeInput\" id=\"" + id + "1\">\r\n<input name=\"" + id
						+ "10\" id=\"" + id + "10" + "\" type=\"time\" oninput=\"onChange(" + id + "10)\" value=\""
						+ startTime + "\"></td>\r\n";

				eredmeny = eredmeny + "<td class=\"timeInput jobbvastag2\" id=\"" + id + "2\">\r\n<input name=\"" + id
						+ "20\" id=\"" + id + "20" + "\" type=\"time\" oninput=\"onChange(" + id + "20)\" value=\""
						+ endTime + "\"></td>\r\n";
				if (i == 5) {
					break;
	...
	protected String teljesTabla(String[][] nevek, String[][] nevekAssz, Map<String, String> myMap)
			throws ServletException, IOException {
		String eredmeny = elsoSor(myMap);
		...
		for (int i = 1; i < 7; i++) {
			...
			eredmeny = eredmeny + teljesSor(1, i, nevek, nevekAssz, myMap);
			eredmeny = eredmeny + teljesSor(2, i, nevek, nevekAssz, myMap);
		...
		eredmeny = eredmeny + buttons.textInputline(true, myMap);
	...
	protected String tablaAlul(Map<String, String> monthMuszak, Map<String, String> monthTime)
			throws ServletException, IOException {
		...
		for (int i = 0; i < nevek.length; i++) {
			eredmeny = eredmeny + "<td id=\"haviMuszakSzam" + nevek[i][1] + "\">" + monthMuszak.get(nevek[i][1])
					+ "</td>\r\n";
		}
		eredmeny = eredmeny + "</tr>";
		eredmeny = eredmeny + "<tr>\r\n <td>Havi idõ</td>\r\n";
		for (int i = 0; i < nevek.length; i++) {
			eredmeny = eredmeny + "<td id=\"haviIdo" + nevek[i][1] + "\">" + monthTime.get(nevek[i][1]) + "</td>\r\n";
		}
		eredmeny = eredmeny + "</tr>";
	...
	protected String selectMenu(String elsoOpcio, int id, String[][] nevek, Map<String, String> myMap)
			throws ServletException, IOException {
		...
		for (int i = 0; i < nevek.length; i++) {
			...
			eredmeny = eredmeny + "<option value=\"" + nevek[i][1] + "\"";
			if (Integer.parseInt(nevek[i][1]) == Integer.parseInt(myMap.get(Integer.toString(id2)))) {
				eredmeny = eredmeny + " selected";
			}
			eredmeny = eredmeny + ">" + nevek[i][0] + "</option>\r\n";
	...
	protected String selectMenuAssz(String elsoOpcio, int id, String[][] nevek, Map<String, String> myMap)
			throws ServletException, IOException {
		// elsoOpció megadja, hogy mit jelenítsen meg alapértelmezettként
		String eredmeny = null;
